...
var apply_wire = function(wheel, letter, input_side, output_side){
...
var apply_wheel = function(message, wheel, reversed = false) {
  let input_side = reversed ? 'side_b' : 'side_a';
  let output_side = reversed ? 'side_a' : 'side_b';
...
var apply_flipped_wheel = (message, wheel)=> apply_wheel(message, wheel, true);
...
var decode = function(message, arrangement) {
  return arrangement
    .reverse()
    .map(get_wheel)
    .reduce(apply_wheel, message);
...
let elements = ["encode", "decode", "input", "output", "wheels"]
...
var transform = function(message, arrangement){
  let fn = elements.encode.checked ? encode : decode;
